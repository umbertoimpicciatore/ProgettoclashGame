 ...
 * @author User
 */
public class PotenziamentoBasatoSuAttaccoStrategy implements PotenziamentoStrategy  {
      @Override
    public ArrayList<Eroe> potenziaEroe(EroeDescrizione desc, Giocatore giocatore) {
        ...
        for (int i=0;i<eroiGiocatore.size();i++){
              ...
              if (s.equals(eroeDaPotenziare)){
                  int attaccoInizialeEroe=eroe.getStat().getAttacco();
                  eroe.getStat().setAttacco(attaccoInizialeEroe+5);
